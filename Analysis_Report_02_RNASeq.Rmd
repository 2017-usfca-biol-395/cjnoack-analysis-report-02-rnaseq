...
# subset it down to just a few here to look for interesting patterns
# in the most highly expressed
sexes_15 <- final_table %>%
  group_by(cancer_stage, gender, smoking_status) %>%
  summarize(mean_age = mean(age_at_diagnosis)) %>%
  arrange(desc(mean_age)) %>%
  head(n = 15)
...
**Table 2**: A table now including smoking status of only females commpared with mean age and cancer stage.
