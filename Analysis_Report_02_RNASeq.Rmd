...
final_table %>%
  ...
  ggplot(aes(x = age_at_diagnosis,
           ...
           geom_line(aes(38, 85, group=ID, colour=ID))
    geom_boxplot() +
    facet_wrap(~cancer_stage) +
    xlab("Gene Name") +
    ylab("Age of diagnosis per gene") +
...
```{r anothertest}
p <- ggplot(data = final_table, mapping = aes(age_at_diagnosis, cancer_stage))
p + geom_point(aes(color = gender)) + geom_smooth()
````
